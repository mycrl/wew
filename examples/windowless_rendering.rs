...
mod webview {
    ...
    use wew::{
        ...
        webview::{
            KeyState, MouseAction, MouseButton, Position, WebView, WebViewAttributes,
            WebViewHandler, WindowlessRenderWebViewHandler,
        },
    };

    use winit::{
        event::{
            ElementState, KeyEvent, Modifiers, MouseButton as WinitMouseButton,
        },
    ...
    impl Webview {
        pub async fn new(message_loop: &MessagePumpLoop, window: Arc<Window>) -> Result<Self> {
            ...
            let webview = runtime
                .async_create_webview(
                    ...
                    WebViewObserver {
                        render: Render::new(window).await?,
                    },
                )
                .await?;
        ...
        pub fn on_mouse_input(&self, state: ElementState, button: WinitMouseButton) {
            self.webview.mouse(&MouseAction::Click(
                match button {
        ...
        pub fn on_mouse_wheel(&self, x: i32, y: i32) {
            self.webview.mouse(&MouseAction::Wheel(Position { x, y }));
        }

        pub fn on_mouse_move(&self, x: i32, y: i32) {
            self.webview.mouse(&MouseAction::Move(Position { x, y }));
        }
    }
}
...
impl ApplicationHandler for App {
    fn resumed(&mut self, event_loop: &ActiveEventLoop) {
        ...
        self.webview.replace(
            RUNTIME
                .block_on(webview::Webview::new(&self.message_loop, window))
                .unwrap(),
        );
    }
